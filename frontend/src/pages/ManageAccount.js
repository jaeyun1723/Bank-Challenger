...
function ManageAccount() {
  ...
  useEffect(() => {
    axios.get(`/withdraw/list/${userId}`)
  .then(response3 => {
  ...
  useEffect(() => {
    ...
    if (code != null) {
      axios.get("http://127.0.0.1:8080/openapi/${userId}/${code}/${clientInfo}").then(window.location.replace("/manageaccount"))
  ...
  function RegisterWithdrawAccount() {
    ...
    return (
        <div style={{ textAlign: 'center' }}>
          <button onClick={registerWAccountClick} style={{ fontSize: '12px', width: '600px', marginLeft: '10px', marginTop: '10px' }} className="btn-1 ml-1 btn btn-outline-info">+ 출금 계좌 추가하기</button>
        </div>
    );
  }
  ...
  function RegisterSavingAccount() {
    ...
    return (
        <div style={{ textAlign: 'center' }}>
          <button onClick={registerSAccountClick} style={{ fontSize: '12px', width: '600px', marginLeft: '10px', marginTop: '10px' }} className="btn-1 ml-1 btn btn-outline-info">+ 저금 계좌 추가하기</button>
        </div>
    );
  }
  ...
  function DeleteWithdrawAccount({ acntId }) {
      ...
      if (window.confirm('삭제하시겠습니까?')) {
        ...
        try {
          axios.delete(/withdraw/${acntId}).then((res) => {
            setWithdrawAccounts(withdrawAccounts.filter((el) => el.accountId !== acntId));
          });
        } catch (error) {
          console.error(error);
        }
    ...
    return (
        <>
          <button onClick={deleteWAccountClick} style={{ fontSize: '12px', padding: '5px 5px', marginRight: '15px', marginTop: '8px' }} className="btn-1 ml-1 btn">
            <img src={require("../assets/img/icons/common/delete_icon.png")}
  ...
  function DeleteSavingAccount({ acntId }) {
      ...
      if (window.confirm('삭제하시겠습니까?')) {
        ...
        try {
          axios.delete(/savings/${acntId}).then((res) => {
            setSavingAccounts(savingAccounts.filter((el) => el.accountId !== acntId));
          });
        } catch (error) {
          console.error(error);
        }
    ...
    return (
        <>
          <button onClick={deleteSAccountClick} style={{ fontSize: '12px', padding: '5px 5px', marginRight: '15px', marginTop: '8px' }} className="btn-1 ml-1 btn">
            <img src={require("../assets/img/icons/common/delete_icon.png")}
                 style={{ width: '20px', height: 'auto' }}
  ...
  const WithdrawList = ({ withdrawAccount }) => {
    ...
    return (
        <div className="withdrawList">
          <div>
            <div style={{ fontSize: '20px', textAlign: 'left', marginBottom: '-10px', marginTop: '-20px' }} className='text-primary ml--4 btn btn-link'>
            ...
            {withdrawAccounts.map((it) => (
                <div style={{ textAlign: 'left' }}>
                  <button
                      type="button"
                      style={{ marginBottom: '5px', textAlign: 'left', width: '600px', display: "flex", }}
                      className="text-primary btn-neutral ml-1 btn btn-primary "
                  >
                    <table style={{ marginRight: "auto" }}>
                      <tr>
                        <th rowSpan='2'><span><img
                            alt="..."
                            className="img-fluid"
                            src={getBankLogo(it.bankName)}
                            style={{ textAlign: 'left', marginBottom: '3px', width: "35px", marginRight: '10px' }}
                        /></span></th>
                        <th style={{ fontSize: '12px', textAlign: 'left', marginRight: '10px' }}>{it.bankName}</th>
                      </tr>
                      <tr>
                        <th style={{ fontSize: '16px', textAlign: 'right' }}>{it.accountNumMasked}</th>
                      </tr>
                    </table>
                    <DeleteWithdrawAccount acntId={it.accountId} style={{ display: "inline-block", marginLeft: 'auto', zIndex: '-1' }} />
                  </button>
                </div>
            ))}
          </div>
            ...
            {savingAccounts.map((it) => (
                <div style={{ textAlign: 'left' }}>
                  ...
                  >
                    ...
                    <DeleteSavingAccount acntId={it.accountId} style={{ display: "inline-block", marginLeft: 'auto', zIndex: '-1' }} />
                  </button>
                </div>
            ))}
          </div>
        ...
        <main id="main">
          <section className="section section-hero section-shaped">
            ...
            <Container style={{
              ...
              <Card style={{ width: "700px" }} className="bg-secondary shadow border-0">
                <CardHeader style={{ textAlign: 'center', width: "700px", marginBottom: '-10px' }} className="bg-white pb-5">
                  <div className="text-muted text-center mb-3">
                    <button
                        type="button"
                        style={{ width: '300px', pointerEvents: 'none', marginBottom: '15px' }}
                        className="btn-1 ml-1 btn btn-outline-danger"
                    >
                      계좌 관리
                    </button>
                  </div >
                  <div style={{ fontSize: '20px', margin: '-20px' }} className="text-danger ml-1 btn btn-link">
